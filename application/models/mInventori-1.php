...
class mInventori extends CI_Model
{
    ...
    function getListHistoryBtbDist($tanggal)
    {
        ...
        $this->db2 = $this->load->database($namedept, true);
        $query = $this->db->query("SELECT a.`szDocId`, c.`szName`, a.`szVehicleId` FROM $base.`dms_inv_docstockindistribution` a
        LEFT JOIN $base.`dms_pi_employee` c ON a.`szEmployeeId` = c.`szId`
        LEFT JOIN $base.`dms_inv_stockadjustmentrefdoc` b ON a.`szDocId` = b.`szAdjustmentId` AND b.`szRefDocTypeId` = 'DMSDocStockInDistribution'
        WHERE a.`dtmDoc` = '$tanggal' AND a.`szBranchId` = '".$this->session->userdata('user_branch')."' AND b.`szRefDocId` IS NULL ORDER BY a.szDocId DESC");
        return $query->result();
        $this->db2->close();
    }
    ...
    function editBtbDistribusi($document)
    {
        ...
        LEFT JOIN $base.`dms_inv_stocktype` g ON a.`szStockType` = g.`szId`
        LEFT JOIN $base.`dms_inv_vehicle` i ON a.`szVehicleId` = i.`szId`
        WHERE a.`szDocId` = '$document'");
        return $query->result();
        $this->db2->close();
    }
    ...
    function getDetMorphing($document)
    {
        ...
        LEFT JOIN dms111asa.`dms_inv_warehouse` b ON a.`warehouse_id` = b.`szId`
        LEFT JOIN wo_admin.`tbl_ckr_morphing_detail` c ON a.`doc_id` = c.`doc_id`
        WHERE a.`doc_id` LIKE '$document%' AND a.`branch_id` = '".$this->session->userdata('user_branch')."'
        GROUP BY a.`doc_id`");
        return $query->result();
        $this->db2->close();
    }
    ...
    function getHistoryMorphing($tanggal)
    {
        ...
        WHERE a.`refDocType` = 'DMSDocStockMorph' AND a.`refDepo` = '".$this->session->userdata('user_branch')."' AND DATE(a.`refDateAdd`) = '$tanggal'");
        return $query->result();
    }
