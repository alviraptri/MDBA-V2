...
{
  public function getLastClosing($depo)
  {
    ...
    $query = $this->db->query("SELECT dtmLastClosing 
    FROM $base.dms_gen_closing 
    WHERE szClosingType = 'CAS' AND szBranchId = '$depo'");
  ...
  public function getDataCashOutForClosing($tanggalNol, $depo)
  {
    if ($this->session->userdata('user_branch') == '321' || $this->session->userdata('user_branch') == '336' || $this->session->userdata('user_branch') == '324') {
      $base = 'dummymdbaasa';
    } else {
  ...
  public function getDataCashInForClosing($tanggalNol, $depo)
  {
    if ($this->session->userdata('user_branch') == '321' || $this->session->userdata('user_branch') == '336' || $this->session->userdata('user_branch') == '324') {
      $base = 'dummymdbaasa';
    } else {
