<?php
class mCashierBtu extends CI_Model
{
    //transaksi
    ...
    function getDetailBtu($id)
    {
        ...
        LEFT JOIN $base.`dms_cas_cashtempbalance` c ON a.`szDocId` = c.`szDocId` AND C.`intItemNumber` = '-1' AND c.`szObjectId` = 'DMSDocCashTempIn'
        LEFT JOIN $base.`dms_fin_account` d ON a.`szAccountId` = d.`szId`
        LEFT JOIN $base.`dms_fin_subaccount` e ON a.`szSubAccountId` = e.`szId`
        LEFT JOIN $base.`dms_cas_doccashtempinitem` f ON a.`szDocId` = f.`szDocId`
