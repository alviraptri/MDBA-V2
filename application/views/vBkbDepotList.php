...
<body>    
                        ...
                        <div class="col-12 col-md-6 order-md-2 order-first">
                            <nav aria-label="breadcrumb" class='breadcrumb-header'>
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page"><a href="<?php echo base_url('home/bkbDepot'); ?>">BKB Depot</a></li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                        ...
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <a href="<?php echo base_url('inventDepot/historyBkb') ?>">
                                        <button type="submit" class="btn btn-secondary">History</button>
                                    </a>
                                    <a href="<?php echo base_url('inventDepot/manualBkb') ?>">
                                ...
                                <tbody>
                                    ...
                                    foreach ($data as $value) { ?>
                                        <tr>
                                            ...
                                            <td><?= $value->status_sa; ?></td>
                                            <td><?= $value->no_pengajuan; ?></td>
                                            <td><?= $value->depo_tujuan; ?></td>
                                            <td>
                                                <input type="hidden" name="a" id="<?= $no; ?>" value="<?= $value->no_pengajuan; ?>">
    ...
    <script>
        function detail(x) {
            ...
            function data() {
                $.ajax({
                    url: "<?= base_url('inventDepot/getDetBkbDepot') ?>",
                    method: "POST",
                    data: {
                        'id': id
                    },
                    ...
                    success: function(data) {
                        console.log(data);
                        for (var row of data) {
                            document.getElementById("idPengajuan").value = row.no_pengajuan;
                            document.getElementById("idTanggal").value = row.tanggal_kebutuhan;
                            document.getElementById("idDepo").value = row.depo_tujuan;
                            document.getElementById("namaPengajuan").value = row.nama_depo;
                            document.getElementById("idPengemudi").value = row.driver;
                            document.getElementById("idKendaraan").value = row.nopol;
                            document.getElementById("idKeterangan").value = row.keterangan;
                        }
                        ...
                        var split = id.split('/');
                        var ref = split[0];
                        var btn = '<a href="<?php echo base_url('inventDepot/tambahBkb/') ?>'+ref+'"><button type="button" class="btn btn-primary">Tambah</button></a>';
                        $('#idTambah').html(btn);
                    },
                });
            }
